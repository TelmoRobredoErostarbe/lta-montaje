...
import { MapPin, CalendarDays, CheckCircle2, Search, Clock } from "lucide-react";
...
export function ViewerEventosPage() {
  ...
  function EventCard({ e }: { e: Evento }) {
    ...
    return (
      <button
        onClick={() => navigate(`/viewer/evento/${e.id}`)}
      ...
      >
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-md border ${badgeClass}`}>{e.formato}</span>
          ...
          <div className="flex items-center gap-3 text-xs text-slate-400 flex-wrap">
            ...
            {e.hora_inicio && <span className="flex items-center gap-1"><Clock size={11} />{e.hora_inicio.slice(0, 5)}</span>}
          ...
          {e.total > 0 && (
            <div className="mt-2.5">
              ...
              <div className="w-full h-1.5 bg-slate-100 rounded-full">
                <div className={`h-1.5 rounded-full transition-all ${done ? "bg-green-500" : "bg-slate-800"}`} style={{ width: `${pct}%` }} />
              </div>
            </div>
          )}
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6">
      <div className="mb-5">
        <h1 className="text-lg font-semibold text-slate-900">Montaje</h1>
        <p className="text-sm text-slate-400 mt-0.5">Estado de los eventos</p>
      </div>
        ...
        <>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-20 text-slate-400 text-sm">
              {search ? "No hay resultados" : "No hay eventos"}
            </div>
          )}
        </>
      )}
